...
Object.prototype.generateID = function () { var id = new Date().getTime() + Math.floor(Math.random() * 100) /*random [0,99]*/; if (this.hasAttribute('id')) { id = this.getAttribute('id'); } else { this.setAttribute('id', id); } return id; }
...
var _cache = new PromiseWorker(new Worker('worker/store.js'));
_cache.postMessage({ hello: 'cache' }).then(function (m) { console.log('CACHE -> UI: ', m); });
...
var ui = {
    ...
    tree: {
        ...
        on_node_click: function (m) {
            ...
            if (el != null) {
                console.log(m);
                var input = m.input, para = (input != null ? input.para : null),
                    //node = Array.form(para).maper('type,path,folder,file'),
                    pa = el.parentElement;
                //console.log(node);
