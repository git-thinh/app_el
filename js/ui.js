...
var get = function (url, event_ok, event_error) { var r = new XMLHttpRequest(); r.onreadystatechange = function () { if (this.readyState == 4 && this.status == 200) { if (event_ok != null) event_ok(JSON.parse(r.responseText)); } }; r.open("GET", url, true); r.send(); }
...
var broadcast; if ('BroadcastChannel' in window) { broadcast = new BroadcastChannel('BROADCAST_ID'); broadcast.addEventListener("message", (e) => { var m = e.data; if (typeof m === 'string') { init_js_css(m); return; } if (m.callback == null) return; if (m.callback.indexOf('___') == -1) window[m.callback](m); else { var a = m.callback.split('.'); if (a.length > 1) { if (window[a[0]] != null && typeof window[a[0]][a[1]] === 'function') { window[a[0]][a[1]](m); } } } }, false); }
...
function start_App() {
    ...
    post_api({ action: 'module_load', input: { code: 'chrome_fetch' } });
