...
api.postMessage({ lik: true, type: 'root', path: '', folder: '', file: '' });
...
var ui = {
    ...
    on_broadcast_message_receiver: function (e) {
        ...
        var m = e.data;
        //if (m.result_type == 'word' || m.result_type == 'text') { console.log(m); return; };
        if (typeof m === 'string') {
        ...
        if (el.hasAttribute('type') && el.getAttribute('type') == 'submit') {
            ...
            if ((el.innerText == 'Cancel' || el.innerText == 'Ok') && el.tagName == 'BUTTON') { ui.dialog.f_cancel(el); } else {
                if (el.hasAttribute('do')) {
                    var elID = el.getOrCreateID();
                    ui.m_elem_current_id = elID;
                    var a = el.getAttribute('do').split(','), input;
                    var m = { callback: a[0], selector: elID };
                    if (a.length > 0) {
                        var index = Number(a[1].trim());
                        if (index != NaN) {
                            //cache.postMessage(index).then((val) => ui.f_callback(a[0], { callback: a[0], selector: elID, input: val }));
                            //input_id = index;
                            ui.post({ action: config.API_CACHE_GET_INPUT, input: index }, function (val) {
                                if (val != null) m.input = val.result;
                                ui.f_callback(m.callback, m);
                            }, ui.dialog.f_alert_show);
                            return;
                        }
                    }
                    ui.f_callback(m.callback, m);
    ...
    f_do: function (func, arrPara, model) {
        if (arrPara.length > 0) {
            if (model != null && model.length > 0) { func += ',' + model; }
            return 'do="' + func + config.SPLIT_KEY + arrPara.join(config.SPLIT_KEY) + '"';
        }
        return 'do="' + func + '"';
    },
    cache: {
            ...
            switch (m.result_type) {
                case 'node':
                    ...
                    if (el != null) {
                        ...
                        var dirs = [];
                        Array.from(rs.dirs).forEach(function (it) { if (it.count > 0) dirs.push({ type: 'dir', path: path, folder: it.name, file: '', count: it.count }); });
                        ui.post({
                            action: config.API_CACHE_RENDER, input: rs.dirs,
                            input: dirs,
                            template: '<li><details><summary do="tree.on_node_click,{{_id}}">{{folder}}({{count}})</summary></li>'
                        }, function (val) {
                            console.log(val);
                        ...
                        //    if (it.count > 0) {
                        //        var ev = _do('tree.on_node_click', { type: 'dir', path: path, folder: it.name, file: '' });
                        //        console.log(ev);
                        //        s += '<li><details><summary ' +
                        //            ev + '>' +
                        //            it.name + '(' + it.count + ')</summary></li>';
                        //    } else {
                        //        s += '<li class="dir-empty">' + it.name + '</li>';
                        //    }
                        //});
                        //Array.from(rs.files).forEach(function (it) {
                        //    var ev = _do('tree.on_node_click', { type: 'file', path: path, folder: '', file: it.name });
                        //    console.log(ev);
                        //    s += '<li class="file" ' +
                        //        ev + '>' + it.title + '</li>';
                        //});
